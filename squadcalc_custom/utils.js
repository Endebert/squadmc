function getKP(lat, lng) {
    ...
    // basic grid, e.g. B5
    let kpCharCode = 65 + Math.floor(x / kp);
    let kpLetter = String.fromCharCode(kpCharCode);
    let kpNumber = Math.floor(y / kp) + 1;

    // sub keypad 1, e.g. B5 - 5
    let subY = Math.floor(y / s1) % 3;

    let subNumber = 10 - ((subY + 1) * 3);

    let subX = Math.floor(x / s1) % 3;

    subNumber += subX;

    // sub keypad 2, e.g. B5 - 5 - 3;
    let sub2Y = Math.floor(y / s2) % 3;

    let sub2Number = 10 - ((sub2Y + 1) * 3);

    let sub2X = Math.floor(x / s2) % 3;
    sub2Number += sub2X;
